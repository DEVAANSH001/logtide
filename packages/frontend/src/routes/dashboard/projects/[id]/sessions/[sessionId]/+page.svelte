<script lang="ts">
	...
	const timeline = $derived.by(() => {
		...
		for (const event of events) {
			...
			// Extract breadcrumbs from metadata
		...
		// Deduplicate breadcrumbs (same message + timestamp can appear in multiple logs)
	...
	// Session stats derived from events
	...
	function getExpandableData(item: TimelineItem): Record<string, unknown> | null {
		...
		if (item.type === 'log' && item.metadata) {
			// Show metadata excluding breadcrumbs (already shown inline)
			...
			<CardContent class="py-4">
				<div class="flex flex-wrap items-center gap-x-6 gap-y-2">
					<div>
						...
						<code class="text-sm font-mono" title={sessionId}>{formatSessionId(sessionId)}</code>
					</div>
					<div>
						<p class="text-xs text-muted-foreground">Service</p>
						<div class="flex gap-1 mt-0.5">
							{#each sessionStats.services as svc}
								<Badge variant="outline" class="text-xs">{svc}</Badge>
							{/each}
						</div>
					</div>
					<div>
						<p class="text-xs text-muted-foreground">Started</p>
						<p class="text-sm">{formatTime(sessionStats.firstEvent)}</p>
					</div>
					<div class="flex items-center gap-1">
						<Clock class="w-3.5 h-3.5 text-muted-foreground" />
						<div>
							<p class="text-xs text-muted-foreground">Duration</p>
							<p class="text-sm">{formatDuration(sessionStats.durationMs)}</p>
						</div>
					</div>
					<div class="flex items-center gap-1">
						...
						<div>
							...
							<p class="text-sm">{sessionStats.eventCount}</p>
						</div>
					</div>
					{#if sessionStats.errorCount > 0}
						<div class="flex items-center gap-1">
							...
							<div>
								...
								<p class="text-sm font-medium text-red-500">{sessionStats.errorCount}</p>
							</div>
						</div>
					{/if}
				</div>
		...
		<div class="relative">
			...
			<div class="space-y-0.5">
				{#each timeline as item (item.id)}
					...
					{#if isLog}
						<!-- Log event — larger card -->
						<div class="relative pl-10 py-1.5">
							<div
								class="absolute left-0 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center {iconConfig.bg} z-10"
							>
								<iconConfig.icon class="w-4 h-4 {iconConfig.color}" />
							</div>

							<div
								class="rounded-md border border-l-2 {getLevelColor(item.level)} p-3 hover:bg-muted/30 transition-colors"
								role="button"
								tabindex="0"
							...
							>
								<div class="flex items-center gap-2 mb-1">
									...
									<Badge variant={getLevelBadgeVariant(item.level)} class="text-[10px] px-1.5 py-0">
										{item.level}
									</Badge>
									{#if item.service}
										<Badge variant="outline" class="text-[10px] px-1.5 py-0">{item.service}</Badge>
									{/if}
									{#if expandable}
										{#if isExpanded}
											<ChevronDown class="w-3 h-3 text-muted-foreground ml-auto" />
										{:else}
											<ChevronRight class="w-3 h-3 text-muted-foreground ml-auto" />
										{/if}
									{/if}
								</div>
								<p class="text-sm {item.level === 'error' || item.level === 'critical' ? 'text-red-600 font-medium' : ''}">{item.message}</p>

								{#if expandable && isExpanded}
									<pre class="mt-2 p-2 bg-muted rounded text-[11px] overflow-x-auto max-h-64">{JSON.stringify(expandable, null, 2)}</pre>
								{/if}
							</div>
						</div>
					{:else}
						<!-- Breadcrumb — compact row -->
						<div class="relative pl-10 py-0.5">
							<div
								class="absolute left-[5px] top-1/2 -translate-y-1/2 w-5 h-5 rounded-full flex items-center justify-center {iconConfig.bg} z-10"
							>
								<iconConfig.icon class="w-3 h-3 {iconConfig.color}" />
							</div>

							<div
								class="flex items-center gap-2 py-1 px-2 rounded hover:bg-muted/30 transition-colors text-muted-foreground"
								role="button"
								tabindex="0"
								onclick={() => expandable && toggleExpanded(item.id)}
								onkeydown={(e) => e.key === 'Enter' && expandable && toggleExpanded(item.id)}
							>
								<span class="text-[11px] font-mono shrink-0">{formatTimestamp(item.time)}</span>
								{#if item.category}
									<span class="text-[10px] font-medium uppercase tracking-wide opacity-60 shrink-0">{item.category}</span>
								{/if}
								<span class="text-xs truncate">{item.message}</span>
								{#if expandable}
									{#if isExpanded}
										<ChevronDown class="w-3 h-3 shrink-0 ml-auto" />
									{:else}
										<ChevronRight class="w-3 h-3 shrink-0 ml-auto" />
									{/if}
								{/if}
							</div>

							{#if expandable && isExpanded}
								<pre class="ml-2 mt-1 p-2 bg-muted rounded text-[11px] overflow-x-auto max-h-48">{JSON.stringify(expandable, null, 2)}</pre>
							{/if}
						</div>
					{/if}
				{/each}
