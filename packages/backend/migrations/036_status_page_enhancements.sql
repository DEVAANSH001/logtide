...
ALTER TABLE projects
  ADD COLUMN status_page_visibility VARCHAR(20) NOT NULL DEFAULT 'disabled'
    CHECK (status_page_visibility IN ('disabled', 'public', 'password', 'members_only')),
  ADD COLUMN status_page_password_hash TEXT;
...
CREATE INDEX idx_status_incidents_project ON status_incidents(project_id, created_at DESC);
CREATE INDEX idx_status_incidents_org ON status_incidents(organization_id);
CREATE INDEX idx_status_incidents_status ON status_incidents(project_id, status);
...
CREATE INDEX idx_status_incident_updates_incident ON status_incident_updates(incident_id, created_at ASC);
...
CREATE INDEX idx_maintenances_project ON scheduled_maintenances(project_id, scheduled_start DESC);
CREATE INDEX idx_maintenances_org ON scheduled_maintenances(organization_id);
CREATE INDEX idx_maintenances_status ON scheduled_maintenances(project_id, status);
CREATE INDEX idx_maintenances_active ON scheduled_maintenances(status, scheduled_start, scheduled_end)
